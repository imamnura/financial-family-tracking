...
export async function GET(request: NextRequest) {
  try {
    ...
    const budgetMap = new Map<string, { id: string; amount: number }>(
      budgets
        .filter(
          (b): b is { id: string; categoryId: string; amount: number } =>
            b.categoryId !== null
        )
    ...
    const realizationMap = new Map<string, number>(
      transactionTotals.map(
        (t: { categoryId: string | null; _sum: { amount: number | null } }) => [
          t.categoryId!,
          t._sum.amount || 0,
        ]
      )
    ...
    const categoriesWithBudget: CategoryWithBudget[] = categories.map(
      (category: {
        id: string;
        name: string;
        type: string;
